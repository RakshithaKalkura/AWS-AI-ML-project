...
def adjust_results4_isadog(results_dic, dogfile):
    ...
    for key, value in results_dic.items():
        ...
        # Check if the results_dic[key] has enough elements
        if len(value) > 1:
            ...
            if value[0] in dognames_dic:
                ...
                if value[1] in dognames_dic:
                    value.extend((1, 1))  # Both are dogs
                else:
                    value.extend((1, 0))  # Pet label is a dog, but classifier label is not

            # Pet Image Label IS NOT a Dog image (e.g. NOT found in dognames_dic)
            else:
                if value[1] in dognames_dic:
                    value.extend((0, 1))  # Pet label is not a dog, but classifier label is

                else:
                    value.extend((0, 0))  # Neither is a dog
        else:
            print(f"Error: Insufficient data for key: {key}, current value: {value}")